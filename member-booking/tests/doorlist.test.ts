...
describe('door list', () => {
  ...
  it('surfaces membership staleness to staff under Pattern B', async () => {
    ...
    await store.members.upsertMany([
      { memberId: 'mem-a', email: 'a@example.com', firstName: 'Ada', lastName: 'Active', status: 'active', homeVenueId: 'east-fremantle', syncedAt: new Date().toISOString(), membershipPackage: null, source: 'hapana' },
    ]);
